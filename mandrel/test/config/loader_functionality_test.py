...
class TestConfigLoaderFunctionality(unittest.TestCase):
    ...
    def testDefaultLoadersList(self):
        self.assertEqual([('yaml', mandrel.config.read_yaml_path)], mandrel.config.LOADERS)
    ...
    def testGetPossibleBasenames(self):
        ...
        mandrel.config.LOADERS = [(a.extension, a), (b.extension, b)]
        
        self.assertEqual(['%s.%s' % (name, a.extension), '%s.%s' % (name, b.extension)],
                         mandrel.config.get_possible_basenames(name))

        mandrel.config.LOADERS.reverse()
        self.assertEqual(['%s.%s' % (name, b.extension), '%s.%s' % (name, a.extension)],
                         mandrel.config.get_possible_basenames(name))

        mandrel.config.LOADERS.reverse()
        self.assertEqual(['%s.%s' % (name, a.extension)],
                         mandrel.config.get_possible_basenames('%s.%s' % (name, a.extension)))
        self.assertEqual(['%s.%s' % (name, b.extension)],
                         mandrel.config.get_possible_basenames('%s.%s' % (name, b.extension)))


    @scenario
    def testFindConfigurationFiles(self):
        with mock.patch('mandrel.util.find_files') as find_files:
            with mock.patch('mandrel.config.get_possible_basenames') as get_possible_basenames:
                exts = [mock.Mock(name='Extension%d' % x) for x in xrange(3)]
                mandrel.config.LOADERS = [(ext, mock.Mock(name='Reader')) for ext in exts]
                mandrel.bootstrap.SEARCH_PATHS = mock.Mock()
                name = mock.Mock(name='FileBase')
                result = mandrel.config.find_configuration_files(name)
    ...
    def testFindConfigurationFileWithMatch(self):
        with mock.patch('mandrel.config.find_configuration_files') as find_configuration_files:
            ...
            result = mandrel.config.find_configuration_file(name)
    ...
    def testFindConfigurationFilesWithoutMatch(self):
        with mock.patch('mandrel.config.find_configuration_files') as find_configuration_files:
            ...
            self.assertRaises(exception.UnknownConfigurationException,
                              lambda: mandrel.config.find_configuration_file('foo'))
    ...
    def testGetLoader(self):
        ...
        mandrel.config.LOADERS = zip(exts, loaders)

        for i in xrange(len(exts)):
            ext, loader = mandrel.config.LOADERS[i]
            path = '%s.%s' % (mock.Mock(name='someFile'), ext)
            result = mandrel.config.get_loader(path)
            self.assertEqual(loader, result)

        self.assertRaises(exception.UnknownConfigurationException,
                          lambda: mandrel.config.get_loader('foo.%s' % mock.Mock(name='NoLoader')))


    @scenario
    def testLoadConfigurationFile(self):
        with mock.patch('mandrel.config.get_loader') as getter:
            path = mock.Mock(name='SomeConfigurationPath')
            result = mandrel.config.load_configuration_file(path)
    ...
    def testGetConfiguration(self):
        with mock.patch('mandrel.config.load_configuration_file') as loader:
            with mock.patch('mandrel.config.find_configuration_file') as finder:
                name = mock.Mock(name='SomeName')
                result = mandrel.config.get_configuration(name)
